...
/// This is the preferred way of using an entitlements instance.  To complete the integration you must call var <code>general</code> func <code>KVAEntitlementsGeneral/registerIdentity(name:identifier:)</code> followed by func <code>start()</code>.  You may alternatively use a constructor to create your own entitlements.  The shared instance simplifies your implementation as you do not need to store an entitlements instance somewhere in a public location in your own code.
...
/// class MyClass
/// {
///     let entitlements = KVAEntitlements(storageIdentifier: "alternate")
/// }
///
/// \endcode\param storageIdentifier An optional storage identifier.  The storage identifier should be left unset (nil) unless you have a reason to not use the default storage space.  See default constructor KVAEntitlements().
///
...
///
/// \endcode
- (void)registerIdentityWithNameString:(NSString * _Nonnull)nameString valueString:(NSString * _Nonnull)valueString SWIFT_DEPRECATED_MSG("Convenience wrapper function deprecated.  Use Swift func general.registerIdentity(name:identifier:) instead.  If in Objective-C then use [KVAEntitlements.shared.general registerIdentityWithName:identifier:]", "general.registerIdentity(name:identifier:)");
...
///
/// \endcode
- (void)unregisterIdentityWithNameString:(NSString * _Nonnull)nameString SWIFT_DEPRECATED_MSG("Convenience wrapper function deprecated.  Use Swift func general.unregisterIdentity(name:) instead.  If in Objective-C then use [KVAEntitlements.shared.general unregisterIdentityWithName:]", "general.unregisterIdentity(name:)");
/// Start the entitlements client.
- (void)start;
/// Start the entitlements client with advanced options.
/// An identity must be registered prior to making this call.  To do this, instead see var <code>general</code> func <code>KVAEntitlementsGeneral/registerIdentity(name:identifier:)</code>.  This method is called by those two methods, and provides public conformance to protocol KVAStartable.  This method can be used to start an instance of a entitlements which was created from decoded JSON.
/// important:
...
/// Stop is similar to <code>invalidate()</code> (or the use of KVAEntitlementsProduct/reset(deleteLocalDataBool:) or KVAEntitlementsProduct.shutdown(deleteLocalDataBool:) which wrap <code>invalidate()</code>) in that both will block new execution, though through different mechanisms;  however, it is different in that stop maintains state and can be started again.  In contrast <code>invalidate()</code> discards state, cannot be re-started, and in the case of the shared instance is automatically released.
...
- (void)stop;
/// Stop the entitlements client with advanced options.
/// The primary action of this method is to reset the startedTask, which will cause any task which is dependent upon that task to block the initiation of new execution.  The current state of the system is fundamentally maintained;  however, essentially the same things which would not execute prior to having been started will not execute while stopped.
/// Stop is similar to <code>invalidate()</code> (or the use of KVAEntitlementsProduct/reset(deleteLocalDataBool:) or KVAEntitlementsProduct.shutdown(deleteLocalDataBool:) which wrap <code>invalidate()</code>) in that both will block new execution, though through different mechanisms;  however, it is different in that stop maintains state and can be started again.  In contrast <code>invalidate()</code> discards state, cannot be re-started, and in the case of the shared instance is automatically released.
/// Stop is similar to class KVANetworking var sleepBool in that both will block new execution through a similar mechanism;  however, it is different in that stop essentially covers all aspects of exectution which are at an inverse of start.  In contrast, class KVANetworking var sleepBool exempts certain key features and these can continue to execute while sleep is in effect.
/// \param logMessagePrintBool A boolean indicating if a log messages consistent with this being a call to a public entry point should be printed.
...
/// Invalidate the entitlements client.
...
- (void)invalidate;
/// Invalidate the entitlements client with advanced options.
...
@interface KVAEntitlements (SWIFT_EXTENSION(KochavaEntitlements)) <JSExport>
@end
...
///
/// \endcode
- (void)registerIdentityWithName:(NSString * _Nonnull)name identifier:(NSString * _Nonnull)identifier;
/// Unregister an identity.
/// <h2>Example</h2>
/// \code
/// KVAEntitlements.shared.general.unregisterIdentity(name: "_SOME_NAME_")
///
/// \endcode
- (void)unregisterIdentityWithName:(NSString * _Nonnull)name;
@end
